...
foreach my $itemToDelete (@ARGV) {
    ...
    if ((substr($itemToDelete, -1) eq '/') and (-l substr($itemToDelete, 0, -1))) {
	# Make sure that the user hasn't tab-completed on a symlink to a directory.
	minorComplaint($itemToDelete, "unclear if you want to delete the symlink or target directory.");
	next;
    }
